...
- Location: columns 6, 7, 8, 9, 10, 11, 36, 37, 38, 39
...
```{python}
pd_raw.loc[:, "Georeferència"].isna().value_counts()
```

```{python}
from datetime import datetime, timedelta

# Pivot the dataframe to have one row per time measurement and as many columns as different air parameters are.
pd_bronze = (
    pd_raw
    .head(25)       # Limit to 25 rows for demonstration purposes, remove for full analysis
    .melt(id_vars=["CODI EOI", "NOM ESTACIO", "DATA", "MAGNITUD", "UNITATS", "TIPUS ESTACIO", "AREA URBANA", 
    "CODI INE", "MUNICIPI", "CODI COMARCA", "NOM COMARCA", "ALTITUD", "LATITUD", "LONGITUD", "Georeferència"],
...
pd_bronze.loc[:, "HORA"] = pd_bronze.loc[:, "HORA"].str.replace("h", "").astype(int)-1

pd_bronze = pd_bronze.assign(
