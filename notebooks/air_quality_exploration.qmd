...
for i in range(0, df_raw.shape[0], max_rows_per_chunk):
    ...
    pivoted_chunk.to_parquet(f"../data/02_intermediate/air_quality_bronze_{index}.parquet", engine="pyarrow")
    index += 1
...
os.remove("../data/02_intermediate/air_quality_bronze_1.parquet")
...
**WIP - check missing data + incorrect values**
...
for i in range(2, index):
    ...
    os.remove(f"../data/02_intermediate/air_quality_bronze_{i}.parquet")
...
If we have a quick check for missing data in the partition of the data that we are working with, we can see that there is some missing data for the main dimension ("VALOR").

```{python}
## Check missing data
import missingno as msno

msno.matrix(df: gdf_bronze)
```