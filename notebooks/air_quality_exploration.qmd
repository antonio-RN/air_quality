...
df_raw = df_raw.iloc[:max_raw_rows]
df_raw.info()
```
...
index = 1
for i in range(0, df_raw.shape[0], max_rows_chunk):
    chunk = df_raw.iloc[i:i+max_rows_chunk,:]
    pivoted_chunk = pivoting_raw_data(chunk)
    pivoted_chunk.to_parquet(data_intermediate.joinpath(f"air_quality_df_{index}.parquet"), engine="pyarrow")
    index += 1

del df_raw, pivoted_chunk, chunk, i, index
```
...
df_bronze = pd.read_parquet(data_intermediate.joinpath("air_quality_df_1.parquet"))
```
...
os.remove(data_intermediate.joinpath("air_quality_df_1.parquet"))
df_bronze.to_parquet(data_intermediate.joinpath("air_quality_df_1.parquet"))
del df_bronze
```
...
def input_missing_data(df_missing: pd.DataFrame) -> pd.DataFrame:
    ...
    df_missing.replace({"ALTITUD": {0: pd.NA}, "LATITUD": {0: pd.NA}, "LONGITUD": {0: pd.NA}}, inplace=True) # clear wrong geopositional data (0 -> NaN)
    ...
    df_merged = pd.merge(df_missing_info, df_correct_info, on=["CODI EOI", "MAGNITUD"], how="left", suffixes=["_x", ""]).drop(columns=["MAGNITUD", "DATA_HORA"]).set_index("index")

    df_clean = df_missing.set_index("index").combine_first(df_merged) # merge both keeping the correct info if available
...
gdf_bronze.to_parquet(data_primary.joinpath("air_quality_bronze_1.parquet"), engine="pyarrow")
del df_bronze_clean, gdf_bronze
```
