...
```{python}
# Import libraries and read parameters
...
# Pivot the data and save as parquet (using dask)
...
del df_raw
dd_pivoted = pivoting_raw_data(dd_raw)
del dd_raw
dd_pivoted.to_parquet(data_intermediate.joinpath("pivoted_logs.parquet"), filesystem="arrow")
del dd_pivoted
```
...
dd_pivoted = dd.read_parquet(data_intermediate.joinpath("pivoted_logs.parquet/part.0.parquet"), filesystem="arrow")
```
...
def transform_datetime(df_old: dd) -> dd:
    ...
    df_new = df_old.assign(
        ...
        df_old.loc[:,"DATA"] + " " + df_old.loc[:,"HORA_tmp"].astype(str), 
        format="%d/%m/%Y %H"
        )
    ).drop(columns=["DATA", "HORA", "HORA_tmp"])
    return df_new

dd_bronze = transform_datetime(dd_pivoted)
del dd_pivoted
dd_bronze.to_parquet(data_intermediate.joinpath("air_log_bronze.parquet"), filesystem="arrow")
del dd_bronze
```

```{python}
dd_bronze = dd.read_parquet(data_intermediate.joinpath("air_log_bronze.parquet/part.0.parquet"), filesystem="arrow").persist()
```
...
def input_missing_data(df_missing: dd) -> dd:
    df_missing = df_missing.dropna(subset=["VALOR"]).reset_index(drop=False).rename(columns={"__null_dask_index__":"index"}) # drop missing measurements
    df_missing = df_missing.replace({"ALTITUD": {0: pd.NA}, "LATITUD": {0: pd.NA}, "LONGITUD": {0: pd.NA}}) # clear wrong geopositional data (0 -> NaN)
    ...
    df_clean = df_missing.set_index("index").combine_first(df_merged).reset_index(drop=True) # merge both keeping the correct info if available
...
dd_bronze_clean = input_missing_data(dd_bronze)
del dd_bronze
dd_bronze_clean.to_parquet(data_intermediate.joinpath("air_log_bronze.parquet"), filesystem="arrow")
del dd_bronze_clean
```

```{python}
dd_bronze_clean = dd.read_parquet(data_intermediate.joinpath("air_log_bronze.parquet"), filesystem="arrow")
dd_bronze_clean.head()
```
...
def create_geodataframe(df_new: dd) -> dask_geopandas.GeoDataFrame:
    # Temporal solution as dd -> dgdf seems not to work
    ...
    gdf_new = dask_geopandas.from_geopandas(gdf_mid)
    return gdf_new

dgdf_bronze = create_geodataframe(dd_bronze_clean.compute())
dgdf_bronze.to_parquet(data_primary.joinpath("air_quality_bronze.parquet"), filesystem="arrow")
del dd_bronze_clean, dgdf_bronze
```

```{python}
# Show first rows of the converted geodataframe
dgdf_bronze = dask_geopandas.read_parquet(data_primary.joinpath("air_quality_bronze.parquet"))
dgdf_bronze.head()
```
...
# Apply transformations to the rest of the chunks and create .parquet bronze files
index = len(glob.glob("*", root_dir=data_intermediate))+1
for i in range(2, index):
    df_old = pd.read_parquet(data_intermediate.joinpath(f"air_quality_df_{i}.parquet"))
    df_new = transform_datetime(df_old)
    df_new = input_missing_data(df_new)
    gdf_new = create_geodataframe(df_new)
    gdf_new.to_parquet(data_primary.joinpath(f"air_quality_bronze_{i}.parquet"), engine="pyarrow")
    os.remove(data_intermediate.joinpath(f"air_quality_df_{i}.parquet"))
    del df_old, df_new, gdf_new
```
...
gdf_bronze = gpd.read_parquet(data_primary.joinpath("air_quality_bronze.parquet"))
